...
#################### Control Structure ##################### Judge## if if [condition]then	[commands]elif	[commands]else	[commands]fi # it is necessary for the bash shell# Loop## for loopfor x in [sequences]; do[commands] # temp var would be specified as $xdone## e.g.for x in {a..z}; doegrep -o 'D. '$x'\S+' dros_list.txt > $x'_species.txt'## while loop# Tips: the variables are separated by the IFSwhile read [local variables]do[contents]done < [input]## e.g.1ls -1 *.txt > myfiles.listcat myfiles.listwhile read myfilenamedoecho -e "Processing $myfilename..."linesinfile=$(wc -l $myfilename | cut -d ' ' -f1)echo -e "\thas $linesinfile lines in it"done <  myfiles.list## e.g.2 count the line in different documentswhile read myfilenamedoecho -e "Processing $myfilename..."linesinfile=$(wc -l $myfilename | cut -d ' ' -f1)echo -e "\thas $linesinfile lines in it"done <  myfiles.list#################### Fucntions ####################function x(){	# $1 and $2 will follow the input order}# 